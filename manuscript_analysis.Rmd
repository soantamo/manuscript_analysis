...
# function for the GAMs (old)
# source(here("function_GAM.R"))

# new function for temp and depth models 
...
# BECAUSE OF TYPO: RE RUN TEMP + DEPTH models from df1-3, df4 still needs to be run & stats overview too

df_1to3_dept_temp <- bind_rows(df_1, df_2, df_3) |> 
  filter(tot_abu > 20)

df_1to3_dept_temp |> 
  distinct(Species)

predictions_depth_temp(df_1to3_dept_temp)
# DO NOT RUN NOW, RUN LATER!
predictions_depth_temp(df_1)
predictions_depth_temp(df_2)
predictions_depth_temp(df_3)
# 4 not run yet, will wait with this
...
plot_all_lms <- df_all_lms|> 
  ...
  scale_fill_manual(values = mycolors,
   ...
   scale_linetype_manual(values = c("endemic" = "solid",
                                   ...
                                   "translocated" = "dashed"), labels = c("endemic", "non-native (ns)", "non-endemic native", "translocated (ns)"))
