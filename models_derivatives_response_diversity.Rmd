...
# with three subfolders: temp, temp_test, depth_temp (total_models/predictions/depth_temp)
...
# saveRDS(no_exclusion_predictions_all, "total_models/df_pred_no_exclusion.rds")
...
no_exclusion_predictions_all <- list.files(path = "total_models/predictions/temp", pattern = ".rds", full.names = TRUE) |>
  map_dfr(readRDS)

# save total predictions as RDS
saveRDS(no_exclusion_predictions_all, "total_models/df_pred_all.rds")
...
for (i in species_list) {
  ...
  for (j in lake_list){
    ...
    # using minimum and maximum temp in the lake for derivatives
     ...
     saveRDS(derivatives, paste0("total_models/derivatives/simple/derivatives_", i, "_",  j, ".rds"))
  }
}
