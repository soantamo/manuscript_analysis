---
title: "02_models_NoElectrofishing"
...
# LOAD DATA
df_models_final <- readRDS('data/df_models_final.rds')
...
lapply(c(3,4,5), function(set_k){
  ...
  output_folder <- paste0('outputs/', model_run, '_k=', set_k)
  ...
  saveRDS(all_gams, file.path(output_folder, 'gam_fits.RDS'))
  ...
  all_preds_long_limits <- all_preds_long_limits %>% 
    mutate(including_electro = F)
  
    saveRDS(all_preds_long_limits %>% 
              mutate(k = set_k), paste0(output_folder, '/temperature_predictions.RDS'))

  
  png(filename = paste0(output_folder, '/thermal_response_curves', '_k=', set_k, '.png'), res = 300, height = 3000, width = 4000)
  print(ggplot(data = all_preds_long_limits %>% filter(!grepl('Salvelinus_sp_Pro', name))) + 
...
# all_temp_preds <- bind_rows(lapply(list.files('outputs', pattern = 'temperature_predictions.RDS', recursive = T, # full.names = T), read_rds)) %>% 
#   filter()
# 
# # plot together all response curves
# png('figures/thermal_responses_together.png', res = 300, height = 3000, width = 4000)
# ggplot(data = all_temp_preds %>% 
#          filter(!grepl('Salvelinus_sp_Pro', name)) %>% 
#          filter(colour == 'inside')) + 
#     geom_line(aes(x = mean_last_7days, y = value, col = paste0(colour, k), group = k)) + 
...
# png('figures/thermal_responses_together_no10.png', res = 300, height = 3000, width = 4000)
# ggplot(data = all_temp_preds %>% 
#          filter(!grepl('Salvelinus_sp_Pro', name)) %>% 
#          filter(colour == 'inside', k != 10)) + 
#     geom_line(aes(x = mean_last_7days, y = value, col = paste0(colour, k), group = k)) + 
#     scale_colour_discrete() + 
#     facet_wrap(~name, scales = 'free_y') + 
#     theme_bw() + 
...
### CONOR REWRITING SCRIPTS ABOVE TO PERFORM SENSITIVITY TESTS
...
df_1 <- readRDS("data/data_frame_models/df_binomial_gam.rds")
df_2 <- readRDS("data/data_frame_models/df_abundance_gam.rds")
df_3 <- readRDS("data/data_frame_models/df_binomial_re.rds")
df_4 <- readRDS("data/data_frame_models/df_abundance_re.rds")
